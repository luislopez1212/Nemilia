<div align="center">
  <img src="screenshots/full_logo.png" alt="Nemilia AI Workspace" width="340" />
  ...
  <img src="https://img.shields.io/badge/version-1.08-gold" alt="version" />
...
**A browser-native AI workspace with multi-agent orchestration, human-in-the-loop review, semantic vector RAG, visual workflow design, predictive execution analysis, and session memory continuity — all in a single HTML file with zero backend.**
...
<div align="center">
  <img src="screenshots/workspace.png" alt="Nemilia workspace folder setup" width="49%" />
  <img src="screenshots/main.png" alt="Nemilia compose interface" width="49%" />
</div>
...
<div align="center">
  <img src="screenshots/Workflow_edit.png" alt="Nemilia workflow editor — live DAG pipeline" width="98%" />
</div>

<br/>

<div align="center">
  <img src="screenshots/workflows.png" alt="Built-in pipeline templates" width="49%" />
  <img src="screenshots/workflow_selection.png" alt="Workflow mode selector" width="49%" />
</div>
...
<div align="center">
  <img src="screenshots/Agents.png" alt="Nemilia built-in agents — SCOUT, LENS, QUILL, FORGE, WEAVE, NEXUS, VISION" width="49%" />
  <img src="screenshots/Agents_new.png" alt="Build custom agents" width="49%" />
</div>
...
<div align="center">
  <img src="screenshots/AI_Provider.png" alt="AI provider configuration — cloud and local" width="49%" />
  <img src="screenshots/webgpu.png" alt="WebGPU offline browser AI" width="49%" />
</div>
...
<div align="center">
  <img src="screenshots/connections.png" alt="Web search and image generation connections" width="98%" />
</div>
...
- **DOCX parsed via ZIP-based XML extraction.** Standard paragraphs and body text fully supported. Embedded charts, macros, and password-protected files are not supported.
...
All libraries load from multiple CDN fallbacks (jsdelivr, esm.run, esm.sh) for reliability. If one CDN is blocked or slow, the next is tried automatically. The three synchronously loaded CDN scripts (jsPDF, Chart.js, PDF.js) are protected with Subresource Integrity (SRI) hashes and `crossorigin="anonymous"`, ensuring the browser rejects any tampered or substituted library file.
...
The three CDN scripts loaded at startup — jsPDF, Chart.js, and PDF.js — carry `integrity` attributes containing SHA-512 hashes and `crossorigin="anonymous"`. The browser computes the hash of every downloaded script and refuses to execute it if the hash does not match. This means a compromised CDN, a BGP hijack, or a man-in-the-middle cannot silently inject malicious code via those script tags.

### CSP Tradeoffs

Nemilia's `Content-Security-Policy` meta tag includes `unsafe-eval` and `unsafe-inline`. These directives are required by the single-file design: the entire application runs as inline JavaScript, and features like Chart.js rendering and dynamic template compilation rely on `eval`-equivalent APIs. The following mitigations reduce the practical risk:

- **`esc()` sanitization** — every piece of user-supplied text is HTML-escaped before DOM insertion, blocking reflected XSS even in the presence of `unsafe-inline`.
- **Color sanitization** — `sanitizeColor()` validates all color values against `/^#[0-9A-Fa-f]{3,6}$/` before they touch the DOM, preventing CSS-injection via crafted color strings.
- **Markdown rendering** — agent outputs are rendered through a controlled Markdown→HTML pipeline, never injected as raw HTML.
- **SRI on CDN scripts** — as described above, the only external scripts are hash-locked.
...
| **Replicate** | Yes (limited) | FLUX Pro · multiple models | [replicate.com](https://replicate.com) |
...
| `Ctrl+K` / `⌘+K` | Focus compose input from anywhere |
...
1. **SRI hashes** — Added `integrity` (SHA-512) and `crossorigin="anonymous"` to the jsPDF, Chart.js, and PDF.js CDN `<script>` tags. Browsers now reject tampered or substituted library files.
2. **DOCX ZIP extractor** — Replaced the naive regex-based DOCX parser with a full ZIP-parsing implementation that reads `word/document.xml` directly, supports both STORE (method 0) and DEFLATE (method 8) via `DecompressionStream`, reconstructs paragraphs from `<w:p>` blocks and `<w:t>` runs, and falls back to raw-text scraping on failure.
3. **`debugLog` utility** — Added `const debugLog = (...a) => { if (localStorage.getItem('nemiliaDebug') === '1') console.log(...a); };` at the top of the main script block. All `console.log()` calls replaced with `debugLog()` so debug output is silent by default and opt-in via `localStorage.nemiliaDebug = '1'`.
4. **Color sanitization** — Added `sanitizeColor(val)` that validates hex colors with `/^#[0-9A-Fa-f]{3,6}$/` and defaults to `#1a3040` on invalid input. Wired into `pickColor()` (on set) and `saveAgentModal()` (on read) to prevent CSS injection via crafted color strings.
5. **Ctrl+K shortcut** — Added a global `document.addEventListener('keydown', ...)` handler that focuses `#compose-input` when `Ctrl+K` (or `⌘+K`) is pressed and the active element is not an input or textarea.
6. **Uncaught try blocks** — Audited all `try {}` blocks; any block without a corresponding `catch` had `catch(e){ console.error('[Nemilia]', e); }` appended to ensure errors are surfaced.
7. **Version string** — All `v1.06` references updated to `v1.08`.


### v1.08 — Initial Release
...
Nemilia AI v1.08 is source-available software, free for personal and non-commercial use.

| Parameter | Value |
|---|---|
| Licensed Work | Nemilia AI v1.08 |
