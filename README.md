<div align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.10-gold" alt="version" />
...
A shortcut button that opens the provider configuration modal. This is where you connect API keys, select models, and manage connections for all supported LLM providers. See the [Provider Reference](#provider-reference) section for a full list of supported services.

---

### Defaults

These values apply to every agent and chat session unless a workflow or agent overrides them individually.
...
A global default system prompt sent to the LLM before every conversation. Individual agents have their own system prompts that override this value during workflow runs. Use this as a baseline persona or rule set for Chat Mode.

**Tip:** Keep this concise — agent system prompts already handle specialist behaviour. This default only matters when using Chat Mode without a specific agent.
...
Controls how many tokens the model is allowed to generate in a single response. This is the `max_tokens` parameter sent to the API — it does **not** control the input context window.

- **Too low** — responses get cut off mid-sentence on long tasks (research reports, detailed analyses).
- **Too high** — costs more and may slow responses on providers that pre-allocate compute.
- **Recommended for workflows:** `4096`–`8192` for standard agents; `16000`+ for VISION when generating complex HTML infographics or detailed SVG diagrams.
- **Recommended for chat:** `2048`–`4096` for most conversations.

**Note:** Modern models (Gemini 1.5 Pro, Claude 3.5, Llama 3.1) support up to 128K–2M token context windows — but that is the *input* context. Max output tokens is a separate, smaller limit set by each provider.
...
Controls how "creative" or "random" the model's output is. Technically, it scales the probability distribution over the model's next-token predictions.

| Value | Behaviour | Best for |
|-------|-----------|----------|
| `0.0` | Fully deterministic — always picks the most likely token | Factual Q&A, code generation, structured JSON output |
| `0.3` | Focused and consistent | Analysis, summarisation, FORGE (critic) |
| `0.7` *(default)* | Balanced creativity and coherence | General writing, research, chat |
| `1.0` | Maximum creativity — more surprising, occasionally incoherent | Brainstorming, creative writing, VISION |

**Tip:** Lower temperature for agents that must follow strict output formats (JSON, structured reports). Higher temperature for agents generating creative or exploratory content.
...
How many times the orchestrator will automatically re-run an agent whose output scored below **6/10** on the quality validation step. Each retry appends the orchestrator's improvement notes to the agent's prompt.

- **1** — no retries; each agent runs once regardless of quality.
- **2–3** — recommended for most workflows. Balances quality with API cost.
- **8** — maximum resilience; useful for critical outputs or low-quality models, but increases API usage significantly.

**Note:** Retries consume additional API tokens. With 5 agents and max retries set to 3, a worst-case run can make up to 15 agent calls instead of 5.
...
The delay inserted **between DAG pipeline stages** (e.g., between Stage 1: research agents completing and Stage 2: analysis agents starting). Additionally, agents *within* the same stage are offset by `stagger ÷ 2` each to avoid simultaneous API bursts.

**Example with default 600ms and a 4-agent mid-stage:**
...
| QUILL | 600ms |
| FORGE | 900ms |
| *(stage boundary)* | +600ms after all complete |
| WEAVE | next stage |

- **0ms** — all agents in each stage fire simultaneously. Fastest, but likely to hit provider rate limits.
- **300–600ms** *(recommended)* — smooth stagger that avoids rate limit errors on most providers.
- **1000ms+** — conservative pacing for providers with strict per-minute request limits (e.g., free-tier Groq or Gemini).

**Tip:** If you are seeing `429 Too Many Requests` errors during workflow runs, increase this value first before looking at other causes.

---

### Storage

Displays the current active storage tier and its status:

| Badge | Storage tier | Notes |
|-------|-------------|-------|
| **FS** | File System Access API | Chrome/Edge only. Workspace synced to a real folder on disk. |
| **IDB** | IndexedDB | Firefox/Safari fallback. No practical size limit. Data survives page refresh. |
| **LS** | localStorage | Last-resort fallback. ~5MB limit. |

The storage info box also shows current usage and available space where the browser exposes that information.

---

### Data

Three workspace management actions:

- **Export JSON** — downloads your entire workspace (agents, prompts, workflows, memory, results, preferences) as a single `.json` file. Use for backup or migrating to a new browser/device.
- **Import** — restores a workspace from a previously exported `.json` file. Merges imported data with your current workspace.
- **Clear all data** — permanently deletes all workspace data from the browser (agents, prompts, workflows, memory, results, chat history, encrypted keys). This action is irreversible. A confirmation dialog appears before deletion.

**Tip:** Export your workspace regularly as a backup, especially before clearing data or switching browsers.
...
1. **Enter-to-Send UX** — All four input areas (Compose, Chat, Followup, Direct Followup) now use `Enter` to send and `Shift+Enter` for newlines. Matches modern chat app conventions and resolves mobile usability issues where `Ctrl+Enter` was inaccessible.

2. **XSS Sanitization** — New `sanitizeVisualHTML()` function strips `<script>` tags, `on*` event handlers, and `javascript:` protocol URLs from all LLM-generated visual outputs (Chart.js, SVG, HTML infographics). The `mdToHtml` renderer also blocks `javascript:` URLs in markdown links/images and adds `rel="noopener"` to external links.

3. **Context Window Management** — Chat messages and workflow followups now use a sliding window that trims older messages when total character count exceeds `3 × maxTokens × CHARS_PER_TOK`. Always preserves the last 2 messages minimum. Prevents hitting provider token limits and reduces API costs on long conversations.

4. **Document Auto-Restore** — Raw file blobs are now stored in IndexedDB at upload time alongside parsed content. On browser reload, if a document's parsed content is missing (e.g. due to localStorage quota pressure), Nemilia automatically re-parses from the stored blob. Documents survive browser restarts even under storage pressure.

5. **Visual Close Buttons** — All rendered visual blocks (charts, SVG diagrams, HTML infographics) now have a hover-reveal `✕` button for removal. Removing a visual updates the PDF capture arrays so exports reflect what the user actually wants. Enables curation before PDF export.

6. **Visual System Cleanup** — Removed unreliable auto-visual triggers: the `wf.media` chart extraction path (Path A) and the `wf.genImage` auto-generation trigger. Removed the legacy `renderCharts()` function (which had a silent empty catch). Removed corresponding `wm-media` and `wm-genImage` workflow modal checkboxes. Visual generation is now exclusively user-initiated via the "Generate Visual" button.

7. **O(N) Hybrid Search** — Replaced O(N²) `.find()` inside `.map()` in `hybridSearch()` with pre-computed `Object.fromEntries()` score maps for O(1) lookups. Measurable improvement on large document corpora.

8. **SLM Punctuation Preservation** — `compressSysPrompt()` now uses `match(/[^.!?]+[.!?]+/g)` instead of `split(/[.!?]+/)`, preserving original sentence-ending punctuation. Questions no longer become statements when prompts are compressed for small local models.

9. **CHARSPERTOK Fix** — Fixed an undefined variable reference (`CHARSPERTOK` → `CHARS_PER_TOK`) that crashed the Predictive Execution Engine's `predictRun()` function.

10. **Storage Failure Notifications** — Silent empty `catch{}` blocks on 5 critical storage paths (localStorage save, IDB workspace, IDB file save, key store, key read) now include `console.warn()` diagnostics. The localStorage quota path now shows a user-facing notification: "Storage full — clear old results or chats to free space."

11. **Mobile Input Fix** — Added `font-size: 16px !important` to all input/textarea/select elements in the mobile media query to prevent iOS Safari auto-zoom on focus.
...
Nemilia AI v1.10 is source-available software, free for personal and non-commercial use.

| Parameter | Value |
|---|---|
| Licensed Work | Nemilia AI v1.10 |
