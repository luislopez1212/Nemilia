<div align="center">
<img width="2816" height="1504" alt="full_logo" src="https://github.com/user-attachments/assets/45aa21ea-742d-46e6-adb7-df1150dabd51" />
  ...
  <img src="https://img.shields.io/badge/version-1.31-gold" alt="version" />
...
- **Unload Model** button releases the engine when switching providers or freeing VRAM
...
| **Custom API** | Any | Any OpenAI-compatible endpoint (vLLM, LMDeploy, Together, Fireworks, etc.) |
...
Any OpenAI-compatible endpoint can be used as a provider — vLLM, LMDeploy, Together AI, Fireworks, LiteLLM, or any self-hosted inference server.

1. Click the provider pill in the header → scroll to **Custom API**.
2. Fill in:
   - **Display Name** — a label shown in the header pill (e.g. `vLLM Local`)
   - **Endpoint URL** — the full base URL (e.g. `http://localhost:8000/v1`)
   - **Model ID** — the exact model string your server expects (e.g. `meta-llama/Llama-3-8B-Instruct`)
   - **API Key** — optional; leave blank if your server doesn't require authentication
   - **Context Window** — optional; set manually if auto-detection fails (e.g. `32768`)
   - **Max Output** — optional; leave blank to use server defaults
3. Click **Save Connection**. The header pill turns green showing your custom provider name.

> **CORS note:** If your server is on a different origin (e.g. `localhost:8000` from a file:// page), ensure your server sends `Access-Control-Allow-Origin: *` headers. Most local inference servers support this with a flag.

> **Format:** Custom providers use the OpenAI chat completions format (`POST /v1/chat/completions`). The endpoint URL should point to the server root, not the completions path.
...
In **Settings → Memory**, a toggle controls whether stored memories are injected into agent context during workflow runs. Turn it off to run a clean session without prior memory context — useful for testing or unrelated tasks. The toggle state persists across sessions.
...
### Local Provider Context Detection

For Ollama, LM Studio, Jan, Llamafile, and Custom API providers, Nemilia auto-detects the context window of the loaded model on connect. LM Studio also supports the native `/api/v0/models` endpoint for richer metadata.

If auto-detection fails or returns an incorrect value, set the context window manually:

1. Open the provider settings → find the **Context Window Override** field.
2. Enter the correct value (e.g. `32768` for a 32K model).
3. Click **Set**. The override persists per-provider and is used immediately.
4. Click **Clear** to revert to auto-detection.

> **Why this matters:** The context window governs how much of the synthesis prompt and agent outputs fit into a single call. An incorrectly low value (e.g. 4096 when the model supports 32K) causes synthesis truncation.

### Local Provider Token Generation

Local providers (Ollama, LM Studio, Jan, Llamafile, Custom API) receive `max_tokens: -1` in all API calls, which both Ollama and LM Studio treat as "generate to end-of-sequence." The model runs to natural completion — no client-side output cap is applied. Context window limits on the server side still apply.
...
**Decomposition UX, Synthesis Reliability & Local Provider Fixes**

1. **Orchestrator Decomposition Visual** — The workflow run now shows two phases before agents start. During decomposition, the status bar displays `🧠 Orchestrator analyzing task…` and each agent card shows a pulsing `Waiting for briefing…` indicator. Once decomposition resolves, the status switches to `📋 Briefing agents…` and each agent card fades in its assigned subtask — the orchestrator's actual plan — for 800ms before execution begins.
2. **Local Providers: Uncapped Token Generation** — Ollama, LM Studio, Jan, Llamafile, and Custom API providers now receive `max_tokens: -1` in all API calls. Both Ollama and LM Studio treat `-1` as generate-to-EOS per their official documentation. Removes a client-side cap (`detectedCtx - 256`) that was silently truncating synthesis output whenever context detection returned a low default (4096).
3. **Synthesis Fallback Restored to v1.21 Logic** — The agent-stitch fallback (which produced `# Synthesis / ## NEXUS / ## SCOUT` output instead of an orchestrated report) was incorrectly firing after every successful stream if `synthesis` was under 100 chars. Fallback now only triggers inside `catch(e)` — on actual stream errors — matching the v1.21 behaviour. Threshold restored from 100 to 50 chars.
4. **Streaming Body Update Unconditional** — The synthesis output body now updates on every streaming chunk regardless of content, matching v1.21. The `if (synthesis)` guard introduced in v1.25 was preventing body updates during model think-blocks, causing a blank panel if the final synthesis was thin after stripping.
5. **Unload Model Button** — The WebGPU "Free GPU Memory" button relabelled to "Unload Model" to accurately reflect what it does (releases the WebLLM engine object). Browser-level GPU memory reclamation is not possible via WebGPU API; a page reload is required for full VRAM release.

---

### v1.30 — 2026-03-06

**Custom Provider, Local Context Detection, Orchestrator Banner & Polish**

1. **Custom API Provider Slot** — A new provider option connects any OpenAI-compatible endpoint. Fields: display name, endpoint URL (required), model ID (required), optional API key, context window, and max output. CORS advisory shown inline. Provider saves to workspace and is available across sessions. API key is optional — servers without authentication are supported.
2. **Orchestrator Animated Banner** — When synthesis begins, a `🎯 ORCHESTRATOR` banner appears at the top of the output panel showing live token count and a pulsing status indicator. On completion, the banner transitions to `✓ Report complete` with a green border before the final report replaces it.
3. **LM Studio Native API Fallback** — Context window detection now falls back to LM Studio's native `/api/v0/models` endpoint when the standard `/v1/models` response contains no context length field (expected — it's not part of the OpenAI spec). The native endpoint returns `context_length`, `max_context_length`, and `n_ctx`.
4. **Manual Context Window Override** — All local provider settings panels now include a context window override field. Enter a value (e.g. `32768`) and click Set to bypass auto-detection entirely. The override persists per-provider in the workspace.
5. **Pre-Run Context Warning** — If the detected context window for a local provider is ≤ 8192 tokens and the workflow has more than one agent, an 8-second orange warning banner advises the user to increase the server's context length to 32K+ for best synthesis results.
6. **Think Toggle Fix** — The reasoning badge (`💭 Reasoning`) was being destroyed on every streaming chunk because `bodyEl.innerHTML` resets removed the prepended wrapper. The badge container now attaches to the card's parent element (`el.parentElement`) rather than `bodyEl`, so it survives innerHTML resets.
7. **Version Strings Wired to `VER` Constant** — Browser tab title, About modal version, and license modal version are all updated dynamically from the single `VER` constant at init.

---

### v1.29 — 2026-03-06

**Per-Provider Context Storage & WebGPU Engine Management**

1. **Per-Provider Detected Context** — Context window values are now stored per-provider (`WS.providers.detectedCtx[pid]`) rather than on a flat `WS.prov.detectedCtx` field. Switching providers no longer bleeds stale context values from the previous provider into the new one.
2. **Always Re-Detect on Save** — Saving a local provider configuration now always re-runs `detectLocalCtx()`, even if the provider was previously configured. The loaded model may have changed since the last save, so the context window is always fresh.
3. **WebGPU Engine Unload on Provider Switch** — When switching away from the WebGPU provider, the WebLLM engine is unloaded automatically, freeing GPU VRAM for other applications.
4. **Migration Shim** — On first load after upgrading from v1.28 or earlier, the flat `detectedCtx` field is migrated into the per-provider store automatically.
...
1. **Local Synthesis Budget** — For local servers and WebGPU, synthesis `max_tokens` is now governed exclusively by `safeOutputBudget(prompt, ctxWindow)` (context window minus input tokens minus 256). Cloud providers continue to apply the user's Max Output Tokens preference as an upper ceiling, since provider APIs enforce their own output limits.
2. **WebGPU Stripped Synthesis Prompt (v1.26 backport)** — WebGPU models receive a shorter synthesis prompt (`~400 chars`) compared to the full guideline prompt sent to cloud/local models. Small-model token budgets are too limited for verbose instruction sets; stripping the prompt to essentials improves synthesis quality.

---

### v1.27 — 2026-03-05

**WebGPU Token Cap & Synthesis Cleanup**

1. **WebGPU Arbitrary Cap Removed** — The 0.45× multiplier on `streamWebLLM` max tokens was removed. WebLLM generates to EOS within the compiled context window naturally; the cap was unnecessarily restricting output length.
2. **Synthesis Echo Cleanup** — Added a post-processing pass that strips the `[…truncated for synthesis]` and `[...truncated for synthesis]` markers from synthesis output if a model echoed the truncation annotation back in its response.

---

### v1.26 — 2026-03-05

**Think Tag Hardening & Gemini Streaming Fix**

1. **Improved `stripThinkingTags`** — Expanded to handle `<think>`, `<thinking>`, `<reasoning>`, and `<scratchpad>` tag variants. Added extraction of content after the last closing tag — when a model writes extensive reasoning then delivers the answer after `</think>`, only the answer is returned. Falls through to full-block strip if the answer is thin.
2. **Gemini Streaming Delta Fix** — Gemini passes the full accumulated text as the `delta` argument (not `full`) in some streaming configurations. Synthesis callback now uses `full ?? delta` to handle both patterns without producing empty or duplicated output.
3. **Synthesis Fallback Threshold** — Raised from 50 to 100 characters to better handle models that produce brief but valid synthesis outputs vs. models that return only think-block content with no actual answer.
4. **Memory Inject Toggle** — Added a separate toggle in Settings controlling whether stored memories are injected into agent context. Previously, disabling the main memory toggle also disabled injection; now injection can be controlled independently.
5. **Strip Think Tags at Storage Time** — Agent outputs are now stripped of think tags before being stored in `agentResults`. This prevents `<think>` blocks from inflating the synthesis prompt with reasoning content, which was consuming token budget and degrading synthesis quality.

---

### v1.25 — 2026-03-05

**Synthesis Diagnostics & Orchestrator Stability**

1. **Synthesis Diagnostics Logging** — After synthesis completes, the run log records both raw output length and stripped output length (e.g. `raw=4823 chars, stripped=3201 chars`). Makes it easy to diagnose think-tag stripping issues.
2. **Safer Synthesis Fallback** — When stripped synthesis is thin, a two-stage recovery attempts: (1) re-strip the complete raw output, catching `</think>
content` patterns; (2) if recovery still fails, fall through to agent stitch. Only the agent stitch path is considered a true failure.

---

### v1.24 — 2026-03-05

**Provider Output Cap & Scroll UX**

1. **Provider Output Cap (`getProviderMaxOutput`)** — New async function queries the real `max_tokens` ceiling for each provider before every `streamAPI` call. For Gemini, queries the `/v1beta/models/{model}` API to get `outputTokenLimit`. For OpenRouter, reads `per_request_limits` or `top_provider.max_completion_tokens` from the cached models list. For all other cloud providers, applies conservative per-provider fallbacks. Prevents 400-class errors on Groq and OpenAI where the API rejects requests exceeding their model's output limit.
2. **Synthesis Scroll-Into-View** — Output panel scrolls into view immediately when synthesis begins, before the first token arrives.

---

### v1.23 — 2026-03-05

**MCP SSE Transport & Context Window**

1. **MCP StreamableHttp SSE Parser** — `mcpCallTool` and `mcpListTools` now handle both plain JSON and SSE response streams from streamableHttp endpoints. Supergateway and other servers that wrap responses in `data: {...}` SSE envelopes are now correctly parsed.
2. **MCP `Accept` Header Fix** — Added `Accept: application/json, text/event-stream` to all MCP requests. Servers that content-negotiate (including Supergateway's streamableHttp mode) were returning 406 Not Acceptable without this header.
3. **Real Context Window for Synthesis** — `_ctxWindow` in the synthesis step now reads from `getProviderCtx()` (the model's actual context window) instead of `parseInt(WS.prefs.maxTokens)` (the user's output preference). For models with large context windows, this allows significantly longer synthesis prompts and outputs.

---

### v1.22 — 2026-03-05

**Context Detection & Per-Model Overrides**

1. **Auto-Detect Context Window on Connect** — Saving a local provider (Ollama, LM Studio, Jan, Llamafile) now triggers `detectLocalCtx()`, which queries the provider's models API for context length metadata (`context_length`, `n_ctx`, `context_window`). The detected value is stored and used in all subsequent token budget calculations.
2. **`getProviderCtx()` — Single Source of Truth** — New helper function returns the active context window for any provider type: uses per-provider stored detection for local providers, `WS.prefs.maxTokens` for cloud providers, and compiled model context for WebGPU.
...
Nemilia AI v1.31 is source-available software, free for personal and non-commercial use.

| Parameter | Value |
|---|---|
| Licensed Work | Nemilia AI v1.31 |
